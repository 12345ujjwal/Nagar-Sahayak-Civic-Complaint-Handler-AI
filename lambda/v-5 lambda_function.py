...
QUESTION_FOR_FIELD = {
    ...
    "location_too_short": "Could you give me your full address — village/town name plus a street or landmark — so the department can actually locate this? A few words isn't enough on its own. Or tap 📍 to share your exact location instead.",
...
Write ONE formal paragraph suitable for submission to a municipal department, in approximately 100 words (do not exceed 120 words). Respond with ONLY the paragraph text, no JSON, no preamble."""
...
def lambda_handler(event, context):
    try:
        ...
        # Without GPS, require a real, detailed address — not just a short place name
        location_too_short = False
        if merged["location"] and not saved_geo and len(merged["location"].strip()) < MIN_ADDRESS_LENGTH:
            merged["location"] = None
            location_too_short = True
        ...
        if missing:
            ...
            question_key = "location_too_short" if (next_field == "location" and location_too_short) else next_field
            save_session(session_id, history, merged, image_keys, saved_geo)
            return {
                "statusCode": 200,
                "headers": CORS_HEADERS,
                "body": json.dumps({
                    "needs_input": True,
                    "session_id": session_id,
                    "message": QUESTION_FOR_FIELD[question_key],
